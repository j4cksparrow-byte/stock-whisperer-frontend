...
const EnhancedStockAnalysis: React.FC = () => {
  ...
  const handleAnalysisSubmit = async () => {
    ...
    try {
      ...
      console.log('[EnhancedStockAnalysis] API response:', res);

      if (res && res.analysis) {
        setLoadingMessage('Processing analysis...');
        const companyName = selectedCompany.name || selectedCompany.symbol;
        const symbol = res.symbol || selectedCompany.symbol;
        const rec = res.analysis?.overall?.recommendation || 'HOLD';
        const score = res.analysis?.overall?.score ?? 50;
        const aiSummary = res.analysis?.aiInsights?.summary || 'Analysis complete. Check individual metrics for details.';
        const currentPrice = res.currentPrice ? `$${res.currentPrice.toFixed(2)}` : '$0.00';
        
        // Extract key factors from technical and fundamental analysis
        const keyFactors = [];
        if (res.analysis.technical?.score >= 70) keyFactors.push('Strong technical indicators');
        if (res.analysis.fundamental?.score >= 70) keyFactors.push('Solid fundamentals');
        if (res.analysis.sentiment?.score >= 70) keyFactors.push('Positive market sentiment');
        
        const risks = [];
        if (res.analysis.technical?.score <= 40) risks.push('Weak technical signals');
        if (res.analysis.fundamental?.score <= 40) risks.push('Concerning fundamentals');
        if (res.analysis.sentiment?.score <= 40) risks.push('Negative sentiment');
        ...
        setAnalysisResult({
          ...
          recommendation: { 
            action: rec, 
            confidence: score >= 70 ? 'HIGH' : score <= 40 ? 'LOW' : 'MEDIUM', 
            score 
          },
          aiSummary,
          keyFactors: keyFactors.length > 0 ? keyFactors : ['Check detailed metrics below'],
          risks: risks.length > 0 ? risks : ['No major risks identified'],
          nextCatalysts: [],
          currentPrice,
          priceContext: res.meta?.dataSource || 'Real-time data'
        });
      } else {
      ...
      console.error('[EnhancedStockAnalysis] Error:', err);
